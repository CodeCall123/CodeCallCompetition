...
const Training = () => {
    ...
    const fetchAudits = async () => {
      try {
        const response = await fetch(
          "https://codecallbackend.vercel.app/training"
        );
        if (!response.ok) {
  ...
  const filteredAudits = audits.filter((audit) => {
    const languageMatch =
      ...
      programmingLanguages.some((lang) => audit.languages.includes(lang));
    const typeMatch =
      types.length === 0 || types.some((type) => audit.types.includes(type));
    const statusMatch =
      statuses.length === 0 || statuses.includes(audit.status.toLowerCase());
    const difficultyMatch =
      difficulties.length === 0 || difficulties.includes(audit.difficulty);
    const searchMatch =
        ...
        <div className="content">
              ...
              return (
                <div className="audit-card" key={audit._id}>
                    ...
                    <div className="audit-price-container">
                      <div className="audit-price">
                        ...
                        <div className="info-tooltip">
                          <strong>
                            Rewards for this contest comprise a pool broken down
                          ...
                          >
                            <div>
                              {" "}
                              {Math.round(audit.points * 0.5)} XP Feature
                            </div>
                            <div>
                              {Math.round(audit.points * 0.1)} XP Optimization
                            </div>
                            <div>
                              {Math.round(audit.points * 0.2)} XP Judging
                            </div>
                  ...
                  <div className="audit-main">
                    <div className="audit-header">
                      ...
                      <div className="audit-tags">
                        ...
                        {audit.types.map((type) => (
                          <span key={type} className={type.replace(/\s/g, "")}>
                            {type}
                          </span>
                        ))}
                        <span
                          className={`tag difficulty ${audit.difficulty.toLowerCase()}`}
                        >
                          {audit.difficulty}
                        </span>
                      </div>
                  ...
                  <div className="view-icon-container">
                    <ChevronRightIcon
                      onClick={() => handleViewClick(audit._id)}
                    />
                  </div>
                </div>
              );
          ...
          <div className="side-info">
            <div className="info-card">
              <h3>Learn</h3>
              <p>
                Courses created by software engineers to help you hone your
                skills and become the best software engineer you can be.
              </p>
              {username ? (
            ...
            <div className="info-card">
              ...
              <p>
                Utilize our training Projects to train your employees in various
                technologies, unlike other platforms..ours is free!
              </p>
              <button
                className="styled-button"
                onClick={() =>
                  (window.location.href =
                    "https://www.codecall.xyz/contact#contact")
                }
              >
                List Project
              </button>
